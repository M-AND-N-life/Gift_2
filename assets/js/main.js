...
function updateCountdown() {
    ...
    const startDate = new Date('2025-11-11T00:00:00'); // Change this to your actual date!
    const now = new Date();
...
window.addEventListener('load', function() {
    bgMusic.play().then(() => {
        ...
        // If autoplay blocked, show paused state
        updateMusicButton();
    });
});
...
function updateMusicButton() {
    if (isMusicPlaying) {
        musicIcon.innerHTML = '🔊';
        musicText.innerHTML = 'إيقاف الموسيقى';
        musicButton.classList.add('playing');
        musicWave.classList.add('playing');
    } else {
        musicIcon.innerHTML = '🔈';
        musicText.innerHTML = 'تشغيل الموسيقى';
        musicButton.classList.remove('playing');
        musicWave.classList.remove('playing');
    }
}

// Update your playVideo function to pause music
function playVideo() {
    ...
    // Your existing video playing code
    const videoContainer = document.querySelector('.video-container');
    if (videoContainer) {
        videoContainer.innerHTML = `
            <video controls autoplay style="width: 100%; border-radius: 15px;" id="myVideo">
...
function boostVolume() {
    ...
    // Set volume multiplier (1.0 is normal, 3.0 is very loud)
